...
import CardPengetahuan from "../../../components/Awal/CardPengetahuan";
...
const Pengetahuan = () => {
  ...
  useEffect(() => {
    const fetchPengetahuan = async () => {
      try {
        const snapshot = await getDocs(collection(db, "pengetahuan"));
        ...
        console.error("Gagal mengambil data pengetahuan:", error);
      }
    };
  ...
  const pengetahuanTersaring = pengetahuan
    .filter((item) =>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 px-4 sm:px-6 lg:px-8 pt-24">
          <div className="max-w-7xl mx-auto">
            {pengetahuanTersaring.length > 0 ? (
              <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {pengetahuanTersaring.map((item) => (
                  <CardPengetahuan key={item.id} data={item} />
                ))}
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center py-20 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-lg font-medium">Tidak ada pengetahuan ditemukan</p>
