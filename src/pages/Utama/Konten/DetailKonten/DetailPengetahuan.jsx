// src/pages/DetailPengetahuan.jsx
...
const DetailPengetahuan = () => {
  ...
  const [pengetahuan, setPengetahuan] = useState(null);
  ...
  useEffect(() => {
    const fetchPengetahuan = async () => {
      try {
        const docRef = doc(db, "pengetahuan", id);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setPengetahuan(docSnap.data());
        }
      } catch (err) {
        console.error("Gagal mengambil pengetahuan:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: pengetahuan?.namaMisi || "Dawnlessday",
          text: "Coba baca pengetahuan ini di Dawnlessday!",
          url: currentUrl,
        });
      } catch (err) {
  ...
  if (!pengetahuan) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p>Pengetahuan tidak ditemukan.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
      ...
      <div className="relative h-80 w-full">
        <img
          src={pengetahuan.gambarMisi}
          alt={pengetahuan.namaMisi}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/70 to-black/30 flex flex-col items-center justify-end pb-6">
          ...
          >
            {pengetahuan.namaMisi}
          </motion.h1>
          <p className="text-gray-200 text-sm mt-2">
            {pengetahuan.tanggal?.seconds
              ? new Date(pengetahuan.tanggal.seconds * 1000).toLocaleDateString(
                  "id-ID"
                )
              : "Tanpa tanggal"}
          </p>
            ...
            <AnimatePresence>
              {showShare && (
                ...
                >
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(pengetahuan?.namaMisi || "Coba baca pengetahuan ini!")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
                  ...
                  <a
                    href={`https://t.me/share/url?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(pengetahuan?.namaMisi || "Coba baca pengetahuan ini!")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
...
export default DetailPengetahuan;