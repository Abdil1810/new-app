  ...
  useEffect(() => {
    const fetchSejarah = async () => {
      try {
        ...
        if (docSnap.exists()) {
          setSejarah(docSnap.data());
        }
      } catch (err) {
        console.error("Gagal mengambil sejarah:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: sejarah?.namaMisi || "Dawnlessday",
          text: "Coba baca sejarah ini di Dawnlessday!",
          url: currentUrl,
        });
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
      ...
      <div className="relative h-80 w-full">
        <img
          src={sejarah.gambarMisi}
          alt={sejarah.namaMisi}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/70 to-black/30 flex flex-col items-center justify-end pb-6">
          ...
          <p className="text-gray-200 text-sm mt-2">
            {sejarah.tanggal?.seconds
              ? new Date(sejarah.tanggal.seconds * 1000).toLocaleDateString(
                  "id-ID"
                )
              : "Tanpa tanggal"}
          </p>
            ...
            <AnimatePresence>
              {showShare && (
                ...
                >
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(sejarah?.namaMisi || "Coba baca sejarah ini!")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
                  ...
                  <a
                    href={`https://t.me/share/url?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent(sejarah?.namaMisi || "Coba baca sejarah ini!")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700"
                  >
