...
export default function AdminSejarah() {
  ...
  const [kontenSejarah, setKontenSejarah] = useState("");
  ...
  const colRef = collection(db, "sejarah");

  // load data
  const loadSejarah = async () => {
    const snapshot = await getDocs(colRef);
    setSejarah(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
  };
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editId) {
      // mode update
      const docRef = doc(db, "sejarah", editId);
      ...
      alert("✅ Sejarah baru berhasil ditambahkan!");
    }
  ...
  const handleDelete = async (id) => {
    await deleteDoc(doc(db, "sejarah", id));
    alert("🗑️ Data sejarah berhasil dihapus!");
    loadSejarah();
  };
  ...
  const handleEdit = (item) => {
    ...
    setKontenSejarah(item.kontenSejarah);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-green-600">Kelola Sejarah</h2>
      ...
      >
        ...
        {openForm ? (editId ? "Tutup Form Edit" : "Tutup Form") : "Tambah Sejarah"}
      </button>
      ...
      {openForm && (
        ...
        >
          ...
          <textarea
            value={kontenSejarah}
            onChange={(e) => setKontenSejarah(e.target.value)}
            ...
            <div className="flex-1">
              ...
              {item.namaMisi && (
                <p className="mt-1 text-xs text-green-600 font-medium">
                  🎯 Misi: {item.namaMisi} ({item.poinMisi} poin, {item.durasi} Detik Baca)
                </p>
              )}

              <div className="mt-2 flex items-center gap-2">
                <span className="text-xs text-gray-400 break-all">ID: {item.id}</span>
                <button
                  onClick={() => {
